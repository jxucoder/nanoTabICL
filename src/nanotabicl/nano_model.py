...
def _target_mlp(X: Tensor) -> Tensor:
    ...
    nf, dev = X.shape[1], X.device
    h = max(4, min(16, nf * 2))
    w1 = torch.randn(nf, h, device=dev) / math.sqrt(nf)
    b1 = torch.randn(h, device=dev) * 0.2
    w2 = torch.randn(h, device=dev) / math.sqrt(h)
...
def _target_linear(X: Tensor) -> Tensor:
    ...
    nf, dev = X.shape[1], X.device
    w = torch.randn(nf, device=dev) / math.sqrt(nf)
    return X @ w + torch.randn(1, device=dev) * 0.2


def _target_tree(X: Tensor, max_depth: int = 4) -> Tensor:
    """Random axis-aligned decision tree target (iterative)."""
    n_rows, n_features = X.shape
    dev = X.device
    values = torch.zeros(n_rows, device=dev)
    stack = [(torch.arange(n_rows, device=dev), 0)]
    while stack:
        indices, depth = stack.pop()
        if depth >= max_depth or len(indices) < 4:
            values[indices] = torch.randn(1, device=dev).item()
            continue
        feat = torch.randint(0, n_features, (1,)).item()
        col = X[indices, feat]
        threshold = col.median()
        left, right = indices[col <= threshold], indices[col > threshold]
...
def _sample_task_torch(
    n_rows: int, n_features: int, device: torch.device,
    prior: Prior = "mlp", task: Task = "classification", n_classes: int = 2,
) -> tuple[Tensor, Tensor]:
    """Generate one synthetic task (classification or regression)."""
    ...
    if task == "classification":
        ...
        return X, torch.bucketize(raw, torch.quantile(raw, quantiles))
...
def _y_encoder(max_classes: int, d: int) -> nn.Linear:
    return nn.Linear(max_classes, d) if max_classes > 0 else nn.Linear(1, d)


def _encode_y(y_enc: nn.Linear, y: Tensor, max_classes: int) -> Tensor:
    if max_classes > 0:
        return y_enc(F.one_hot(y.long(), max_classes).float())
    return y_enc(y.unsqueeze(-1).float())
...
class _MHA(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        o = F.scaled_dot_product_attention(
            self._reshape_heads(self.wq(q)),
            self._reshape_heads(self.wk(k)),
            self._reshape_heads(self.wv(v)),
...
class _Block(nn.Module):
    ...
    def forward(
        self, src: Tensor, *, ctx: Tensor | None = None, train_size: int | None = None,
    ) -> Tensor:
        h = self.ln(src)
        kv = self.ln(ctx) if ctx is not None else (h[..., :train_size, :] if train_size is not None else h)
...
class _ISAB(nn.Module):
    ...
    def __init__(self, d: int, nhead: int, ff: int, n_ind: int, dropout: float = 0.0):
        super().__init__()
    ...
    def forward(self, src: Tensor, train_size: int | None = None) -> Tensor:
        ...
        return self.s2(src, ctx=self.s1(I, ctx=ctx))


class _ColEmbedding(nn.Module):
    """Stage 1: distribution-aware column embedding via shared ISAB + affine output."""
    ...
    def __init__(self, cfg: NanoTabICLConfig):
        ...
        self.y_enc = _y_encoder(cfg.max_classes, d)
        self.isabs = nn.ModuleList(
            [_ISAB(d, cfg.nhead, ff, cfg.col_num_inds, cfg.dropout)
             for _ in range(cfg.col_num_blocks)]
        )
        self.w_proj, self.w_ln = nn.Linear(d, d), nn.LayerNorm(d)
        self.b_proj, self.b_ln = nn.Linear(d, d), nn.LayerNorm(d)
    ...
    def forward(self, X: Tensor, y_train: Tensor, train_size: int) -> Tensor:
        ...
        src[:, :, :train_size] = (
            src[:, :, :train_size]
            + _encode_y(self.y_enc, y_train, self.max_classes).unsqueeze(1)
        )
        d = src.shape[-1]
        ...
        src = src.reshape(B, H, T, d)
        return (feats * self.w_ln(self.w_proj(src)) + self.b_ln(self.b_proj(src))).permute(0, 2, 1, 3)


class _RowInteraction(nn.Module):
    """Stage 2: feature interaction within rows + CLS-token aggregation."""
    ...
    def __init__(self, cfg: NanoTabICLConfig):
        ...
        self.blocks = nn.ModuleList(
            [_Block(d, cfg.nhead, ff, cfg.dropout) for _ in range(max(0, cfg.row_num_blocks - 1))]
        )
    ...
    def forward(self, emb: Tensor) -> Tensor:
        B, T, H, d = emb.shape
        C = self.n_cls
        tokens = torch.cat([self.cls.expand(B, T, C, d), emb], dim=2).reshape(B * T, H + C, d)
        for blk in self.blocks:
            tokens = blk(tokens)
        return self.out_ln(self.cls_block(tokens[:, :C], ctx=tokens)).reshape(B, T, C * d)


class _ICLearning(nn.Module):
    """Stage 3: dataset-level ICL with train-only context attention + MLP decoder."""
    ...
    def __init__(self, cfg: NanoTabICLConfig):
        ...
        self.max_classes = cfg.max_classes
        self.y_enc = _y_encoder(cfg.max_classes, icl_d)
        out_dim = cfg.max_classes if cfg.max_classes > 0 else 1
        self.blocks = nn.ModuleList(
            [_Block(icl_d, cfg.nhead, ff, cfg.dropout) for _ in range(cfg.icl_num_blocks)]
        )
    ...
    def forward(self, R: Tensor, y_train: Tensor) -> Tensor:
        ts = y_train.shape[1]
        R = R.clone()
        R[:, :ts] = R[:, :ts] + _encode_y(self.y_enc, y_train, self.max_classes)
        for blk in self.blocks:
...
class NanoTabICL(nn.Module):
    ...
    def forward(self, X: Tensor, y_train: Tensor) -> Tensor:
        """X: (B, T, H), y_train: (B, train_size) -> (B, test_size, out_dim)."""
        ts = y_train.shape[1]
...
def pretrain(model: NanoTabICL, cfg: PretrainConfig | None = None) -> list[float]:
    ...
    for step in range(1, cfg.steps + 1):
        ...
        if nr - ts < 1:
            continue
        prior = priors[torch.randint(0, len(priors), (1,)).item()]
        Xs, Ys = [], []
        for _ in range(cfg.batch_size):
            ...
            x, y = _sample_task_torch(
                nr, nf, dev, prior=prior,
                task="classification" if is_cls else "regression", n_classes=nc,
            )
            Xs.append(x)
            Ys.append(y)
...
def _zscore(X: Tensor, mu: Tensor, sd: Tensor) -> Tensor:
    return (X - mu) / sd


def _load_or_build(
    config: NanoTabICLConfig | None, checkpoint: str | None,
    device: torch.device, default_max_classes: int,
) -> NanoTabICL:
    """Load from checkpoint or build from config."""
    if checkpoint:
        return load_checkpoint(checkpoint, str(device))
    cfg = config or NanoTabICLConfig()
    if default_max_classes == 0:
        cfg.max_classes = 0
    elif cfg.max_classes < 1:
        cfg.max_classes = default_max_classes
    return NanoTabICL(cfg)
...
class NanoTabICLClassifier:
    ...
    def __init__(
        self, config: NanoTabICLConfig | None = None, checkpoint: str | None = None,
        temperature: float = 0.9, device: str = "auto",
    ):
        self.temperature = temperature
        self.device = _resolve_device(device)
        self.model = _load_or_build(config, checkpoint, self.device, 10)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> NanoTabICLClassifier:
        ...
        self._X = _zscore(X_t, self._x_mu, self._x_sd).to(self.device)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        Xn = _zscore(_to_tensor(X), self._x_mu, self._x_sd).to(self.device)
        logits = self.model(torch.cat([self._X, Xn]).unsqueeze(0), self._y.unsqueeze(0))
        return F.softmax(logits[0, :, :self._n_cls] / self.temperature, dim=-1).cpu().numpy()
...
class NanoTabICLRegressor:
    ...
    def __init__(
        ...
        self.device = _resolve_device(device)
        self.model = _load_or_build(config, checkpoint, self.device, 0)
        self.model.to(self.device).eval()
        self._fitted = False
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> NanoTabICLRegressor:
        ...
        self._X = _zscore(X_t, self._x_mu, self._x_sd).to(self.device)
        self._y = _zscore(y_t, self._y_mu, self._y_sd).to(self.device)
        self._fitted = True
        return self
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        if not self._fitted:
            raise RuntimeError("Call fit first")
        Xn = _zscore(_to_tensor(X), self._x_mu, self._x_sd).to(self.device)
        out = self.model(torch.cat([self._X, Xn]).unsqueeze(0), self._y.unsqueeze(0))
        return (out[0, :, 0] * self._y_sd + self._y_mu).cpu().numpy()
